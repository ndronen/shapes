...
def generate_object_spec(shape_choices, *, textures=None, colors=None,
    ...
    # Hack for Tessa's instance segmentation model.
    if random_state.choice([False, True]):
        # The building class.
        shape_class = 1
    else:
        # The instance ID of a parking lot.
        shape_class = max(instance_ids)+1
        instance_ids.add(shape_class)
...
def instance_random_shapes(
        image_shape, max_shapes, *, min_shapes=1, min_size=2,
        ...
        fill_is=None, colors=None, textures=None, background_texture=None):
    """
        ...
        for trial_num in range(num_trials):
            ...
            if allow_overlap or not filled[mask].any():
                ...
                image = overlay_object(
                    image, target, filled, mask, object_spec)
                labels.append(label)
                break
        else:
